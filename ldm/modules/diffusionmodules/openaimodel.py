...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, semantic=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                assert context is not None and semantic is not None
                x = layer(x, context, semantic)
            else:
                x = layer(x)
        return x
...
class ResBlockSFT(TimestepBlockDual):
    def __init__(
        self,
        channels,
        emb_channels,
    ...
    def __init__(
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SpatialTransformer(  # uses double cross-attention
                            ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
...
class EncoderUNetModelWT(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        input:
            ...
            timesteps: //
        function:
            `get 'sacle' and 'shift' with timesteps
            `transform multi-level features of "x" with scale and shift
        next:
            How does denoising U-net use the output of this model?(struc_c=structcond_stage_model(x, t))
        """
