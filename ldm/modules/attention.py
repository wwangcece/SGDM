...
class BasicTransformerBlockV2(nn.Module):
    ...
    def forward(self, x, context=None, semantic=None):
        return checkpoint(self._forward, (x, context, semantic), self.parameters(), self.checkpoint)

    def _forward(self, x, context=None, semantic=None):
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None) + x
        x = self.attn2(self.norm2(x), context=context) + x
        x = self.attn3(self.norm4(x), context=semantic) + x
        x = self.ff(self.norm3(x)) + x
        return x
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 ...
                 disable_self_attn=True, use_linear=False,
                 use_checkpoint=True):
        super().__init__()
...
class SpatialTransformerV2(nn.Module):
    ...
    def forward(self, x, context=None, semantic=None):
        ...
        if self.use_linear:
            x = self.proj_in(x)
        for i, block in enumerate(self.transformer_blocks):
            x = block(x, context=context[i], semantic=semantic[i])
        if self.use_linear:
            x = self.proj_out(x)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        if not self.use_linear:
