...
class DDPM(pl.LightningModule, ImageLoggerMixin):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert parameterization in ["eps", "x0", "v", "x0_pixel"], 'currently only supporting "eps" and "x0" and "v"'
    ...
    def shared_step(self, batch):
        x = self.get_input(batch, self.hr_key)
...
class LatentDiffusion(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            x = super().get_input(batch, self.hr_key)
            x = x.to(self.device)
    ...
    def shared_step(self, batch, **kwargs):
        x, c = self.get_input(batch, self.hr_key)
        loss = self(x, c)
        return loss
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=50, ddim_eta=0., return_keys=None,
        ...
        log = dict()
        z, c, x, xrec, xc = self.get_input(batch, self.hr_key,
...
#     def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None, struct_cond=None):
