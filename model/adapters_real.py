...
class My_Adapter(nn.Module):
    def __init__(
        ...
        self.channels = channels
        self.nums_rb = nums_rb
        self.merge_encoder = AdaIN_Encoder(320)
        self.conv_in = nn.Conv2d(320, channels[0], 3, 1, 1)
        self.body = []
        for i in range(len(channels) - 1):
            for j in range(nums_rb):
                if j == 0:
    ...
    def forward(self, SR, Ref, HR, if_sample_style=False, style_mu=None, style_std=None):
        # unshuffle
        x = self.merge_encoder(SR, Ref, HR, if_sample_style=if_sample_style, style_mu=style_mu, style_std=style_std)
        # extract features
        features = []
        x = self.conv_in(x)
        features.append(x)
