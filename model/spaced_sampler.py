...
class SpacedSampler:
    ...
    def sample_with_mixdiff(
        ...
        cond_sr_img = cond['cond_sr'][0]
        cond_ref_img = cond['cond_ref'][0]
        # sampling loop
        for i, step in enumerate(iterator):
            ...
            for hi, hi_end, wi, wi_end in tiles_iterator:
                ...
                tile_cond_img = cond_sr_img[:, :, hi * 8:hi_end * 8, wi * 8: wi_end * 8]
                tile_cond_ref_img = cond_ref_img[:, :, hi * 8:hi_end * 8, wi * 8: wi_end * 8]
                tile_cond_latent = self.model.apply_cond_ref_encoder(tile_cond_img, tile_cond_ref_img)

                tile_cond = dict(
                    c_crossattn=cond['c_crossattn'],
                    cond_latent=[tile_cond_latent],
                    lq=[tile_cond_img],
                )
        ...
        img_buffer = torch.zeros_like(cond_sr_img)
        count = torch.zeros_like(cond_sr_img, dtype=torch.long)
    ...
    def sample(
        ...
        steps: int,
        shape: Tuple[int],
        cond_img: Dict[str, torch.Tensor],
        positive_prompt: str="",
        negative_prompt: str="",
        x_T: Optional[torch.Tensor]=None,
        cfg_scale: float=1.,
        ...
        # apply color correction (borrowed from StableSR)
        if color_fix_type == "adain":
            img_pixel = adaptive_instance_normalization(img_pixel, cond_img["lq"][0])
        elif color_fix_type == "wavelet":
            img_pixel = wavelet_reconstruction(img_pixel, cond_img["lq"][0])
        else:
            assert color_fix_type == "none", f"unexpected color fix type: {color_fix_type}"
        return img_pixel
