...
from .vgg16 import SR_Encoder, Ref_Encoder
from utils.metrics import calculate_psnr_pt, LPIPS

# Do forward process for UNetModel with prepared "control" tensors
class ControlledUnetModel(UNetModel):
    # def encode(self, x, timesteps=None, context=None):
    #     '''
    #         x: Zt
    #         context: flattened feature maps from street map [tensor(B N context_dim)]
    #     '''
    #     hs = []
    #     with torch.no_grad():
    ...
    #             h = module(h, emb, context[i])
    #             hs.append(h)
    #         h = self.middle_block(h, emb, context[-1])
    #         hs.append(h)
    #     return hs
    
    # def decode(self, timesteps=None, hs=None, control=None):
    #     '''
    #         hs: output of every level of encoder
    #         control: flattened output of controlnet [tensor(B N context_dim)]
    #     '''
    #     with torch.no_grad():
    #         t_emb = timestep_embedding(
    #             timesteps, self.model_channels, repeat_only=False
...
class ControlNet(nn.Module):
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [
                TimestepEmbedSequential(
                    conv_nd(dims, hint_channels * 2, model_channels, 3, padding=1)
                )
            ]
        )
        '''
            self.input_blocks: 0:(32)[conv]
            1~3: (32)[res attn] [res attn] [res down] 
            4~6: (16)[res attn] [res attn] [res down] 
            7~9: (8)[res attn] [res attn] [res down] 
            10~11: (4)[res res]
        '''
        # True
    ...
    def forward(self, x, cond_sr, cond_ref, timesteps, context, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
        # TODO: use conv to erase the domain gap between vectormap and image
        outs = []

        x = torch.cat((x, cond_sr), dim=1)
        # x = cond_ref + x
...
class ControlLDM(LatentDiffusion):
    def __init__(
        ...
        **kwargs,
        ...
        self.sr_ref_encoder = SR_Encoder()
        if self.frozen_diff:
            self.model.eval()
    ...
    def apply_cond_ref_encoder(self, control_sr):
        return self.sr_ref_encoder(control_sr * 2 - 1) * self.scale_factor
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        sr_cond_latent = self.apply_cond_ref_encoder(sr_cond)
        ref_cond_latent = F.interpolate(ref_cond, scale_factor=1.0/8.0)

        return x, dict(
            c_crossattn=[c],
            sr_cond_latent=[sr_cond_latent],
            ref_cond_latent=[ref_cond_latent],
            lq=[lq],
        )
    ...
    def apply_model(self, x_noisy, t, cond, *args, **kwargs):
        ...
        control = self.control_model(
            x=x_noisy,
            cond_sr=cond["sr_cond_latent"][0],
            cond_ref=cond["ref_cond_latent"][0],
            timesteps=t,
            context=cond_txt,
        )
    ...
    def configure_optimizers(self):
        ...
        params = list(self.control_model.parameters()) + list(self.sr_ref_encoder.parameters())
