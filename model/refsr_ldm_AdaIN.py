...
class ControlLDM(LatentDiffusion):
    def __init__(
        ...
        **kwargs,
        ...
        # 加载SR_Ref_Encoder模型
        self.sr_ref_encoder = AdaIN_Encoder()
    ...
    def apply_cond_ref_encoder(self, control_sr, control_ref, control_hr):
        results = self.sr_ref_encoder(control_sr * 2 - 1, control_ref * 2 - 1, control_hr, self.if_sample_style, self.style_mean, self.style_std)
        # return  [results[0]*self.scale_factor, results[1], results[2]]
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        sr_ref_cond_latent = self.apply_cond_ref_encoder(sr_cond, ref_cond, hr_cond)
    ...
    def validation_inference(self, batch, batch_idx, save_dir, if_sample_style=False, style_mean=None, style_std=None):
        # 开始测试
    ...
    # def style_loss(self, style_mean, style_std):
    #     return torch.sum(0.5 * (style_std**2 - (2 * torch.log(style_std) + 1) + style_mean**2))
    ...
    #     # style loss
    #     weight = 1e-3
    #     style_mean, style_std = cond["style_mean"][0], cond["style_std"][0]
    #     style_loss = self.style_loss(style_mean, style_std)
    #     loss += weight * style_loss
